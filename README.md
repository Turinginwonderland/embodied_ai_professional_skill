...
/plugin marketplace add https://github.com/Turinginwonderland/embodied_ai_professional_skill.git
...
**为何用完整 URL 而非 `owner/repo` 简写**：实测发现 `add Turinginwonderland/embodied_ai_professional_skill` 简写在当前 Claude Code 版本下走 SSH 协议，SSH 配错或 key 缺失会失败。改用 `https://...` 完整 URL 走 HTTPS，对认证要求最低（GitHub 凭据即可），团队成员无需配 SSH key。
